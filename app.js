...
form.addEventListener("submit", async (e)=>{
  ...
  const writeInVisibilityThreshold = out.writeInVisibilityThreshold || 5;
  const progress = out.leaderboardEligible === false
    ? `<div class="mini">${count}/${writeInVisibilityThreshold} counted write-in votes to appear on the leaderboard.</div>`
    : count < THRESHOLD
