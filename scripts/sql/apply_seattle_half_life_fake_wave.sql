drop table if exists tmp_seattle_half_life_fake_wave_updated;

create temporary table tmp_seattle_half_life_fake_wave_updated as
with targets as (
  select
    v.id,
    case
...
updated as (
  ...
  set vote_status = 'rejected',
      is_valid_vote = false,
      invalid_reason = 'manual_seattle_half_life_fake_wave_2026_03_21',
      status_reason_code = 'manual_review_rejected',
      status_reason_detail = 'Seattle final audit: reject exact fake-domain and troll-identity Half Life wave rows',
      status_changed_at = now(),
      status_changed_by = 'manual:seattle_half_life_fake_wave_20260321'
  from targets t
  where v.id = t.id
    and t.target_reason is not null
  returning
    v.id,
    v.created_at,
...
from tmp_seattle_half_life_fake_wave_updated
...
order by rows_removed desc, email_domain;
